...
fn readable_tempdir() -> TempDir {
    // The path decoder enumerates each ancestor. VPS-style TMPDIR parents can
    // be execute-only, so place these fixtures under the readable checkout.
