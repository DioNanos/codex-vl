...
/// The retained-models guard on its own: here the DESTINATION is valid, so the
/// only guard that can refuse the switch is the loop over the retained models.
/// The test above always makes the destination oversized, which fires the first
/// guard before the loop is ever reached.
#[tokio::test]
async fn model_safety_rejects_oversized_retained_parent_fallback_messages() {
    let (_, turn) = make_session_and_context().await;
    let (admitted, destination) = safety_models();
    let (_, mut oversized_retained) = safety_models();
    let oversized = "x".repeat(8 * 1024 + 1);
    parent_review_messages(&mut oversized_retained).node_repl_policy = Some(oversized);
    let error = check_legacy_model_safety(
        &admitted,
        &oversized_retained,
        &destination,
        &turn.config,
    ...
    .expect_err("an oversized retained model must refuse the switch");
    assert!(
        error.contains("field `node_repl_policy` is 8193 bytes; maximum is 8192 bytes"),
        "unexpected error: {error}"
    );
