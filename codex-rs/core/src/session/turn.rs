...
pub(super) fn realtime_text_for_event(msg: &EventMsg) -> Option<(String, Option<MessagePhase>)> {
    match msg {
        ...
        // codex-vl: realtime/voice also surfaces streaming deltas; AgentMessageDeltaEvent
        // has no phase field, so report None.
        EventMsg::AgentMessageDelta(event) => Some((event.delta.clone(), None)),
