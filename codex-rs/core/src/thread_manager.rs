...
/// requests that do not own a full [`ThreadManager`] session.
///
/// Keeping authentication and model-manager construction inside `codex-core`
/// prevents UI crates from depending on those manager implementation details.
pub async fn build_background_model_client(
    config: &Config,
    model_name: &str,
    session_source: SessionSource,
) -> (ModelClient, ModelInfo) {
