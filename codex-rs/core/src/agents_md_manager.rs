...
impl AgentsMdManager {
    ...
    pub(crate) async fn refresh(
        ...
        let (mut instructions, cached, refresh_repository) = {
            ...
            // The environment selection and the project trust level are not the
            // only inputs: the files on disk are one too. A session that started
            // without AGENTS.md and then had one created — which is exactly what
            // `/init` does — kept reporting that none existed, because the
            // selection had not changed (codex-termux#14). Re-run discovery while
            // nothing has been found; it is a bounded set of path probes, and it
            // stops costing anything as soon as instructions exist.
