...
impl CodexResponsesMetadata {
    // codex-vl: public so detached background clients (Vivling) can build a
    // per-request metadata snapshot. Upstream moved installation/session ids out
    // of `ModelClient::new` into this per-turn snapshot (rust-v0.140.0); Vivling
    // has no turn graph, so it constructs a minimal snapshot (request_kind = None
    // => no turn-metadata header, matching the prior `turn_metadata_header: None`).
    pub fn new(
