...
#[tokio::test]
async fn background_model_client_resolves_requested_model() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codex_home = temp_dir.path().join("codex-home").abs();
    config.cwd = config.codex_home.abs();
    std::fs::create_dir_all(&config.codex_home).expect("create codex home");
    let model_name = config.model.clone().expect("test model");

    let (_client, model_info) = build_background_model_client(
        &config,
        &model_name,
        SessionSource::Custom("background-model-client-test".to_string()),
    )
    .await;

    assert_eq!(model_info.slug, model_name);
}
