...
// codex-vl fork: client name recognized for the manage_loops builtin grant
// (`client_requests_manage_loops_builtins`), kept beside the fork constants.
...
fn with_builtin_dynamic_tools(mut tools: Vec<DynamicToolSpec>) -> Vec<DynamicToolSpec> {
    ...
    let mut builtin_namespace_injected = false;
    for tool in &mut tools {
        ...
        if !builtin_namespace_injected {
            namespace.tools.push(DynamicToolNamespaceTool::Function(
                manage_loops_dynamic_function_spec(),
            ));
            builtin_namespace_injected = true;
        }
    }

    tools.push(DynamicToolSpec::Function(
        manage_loops_dynamic_function_spec(),
    ));
    if !builtin_namespace_injected {
...
impl ThreadRequestProcessor {
    ...
    async fn thread_resume_inner(
        ...
        if let ThreadResumeTarget::Client(request_id) = target
            && let Some(source_thread) = resume_source_thread.as_ref()
        {
            self.thread_state_manager
        ...
        let response_history = thread_history.clone();
        let mcp_binding_context = match target {
