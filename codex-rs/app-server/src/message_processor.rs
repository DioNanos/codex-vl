...
use crate::identity::{AuthorityVerifier, ConnectionIdentityState};
...
pub(crate) struct MessageProcessor {
    pub(crate) authority_verifier: Option<std::sync::Arc<dyn AuthorityVerifier>>,
...
// No Debug: the authority verifier (trait object) is not Debug and
// representing it as a string has no diagnostic value.
pub(crate) struct ConnectionSessionState {
    ...
    /// Authority verifier (verify v1 channel). `None` in production until
    /// the channel is wired: identity_required ON is then fail-closed
    /// (IdentityUnverified), never accept-everything.
    pub(crate) authority_verifier: Option<std::sync::Arc<dyn AuthorityVerifier>>,
}
...
impl ConnectionSessionState {
    ...
    /// Wires the real verifier onto the verify v1 channel. `None`
    /// (production until the channel is wired) = fail-closed when ON.
    pub(crate) fn set_authority_verifier(
        &mut self,
        verifier: Option<std::sync::Arc<dyn AuthorityVerifier>>,
    ) {
        self.authority_verifier = verifier;
    }
    ...
    pub(crate) fn identity_challenge(
        &self,
    ) -> Option<codex_app_server_protocol::IdentityChallenge> {
        self.identity
            .lock()
            .ok()
            .and_then(|state| state.challenge())
    }
    ...
    pub(crate) async fn bind_identity(
        ...
        verifier: Option<&dyn AuthorityVerifier>,
    ) -> Result<
        codex_app_server_protocol::IdentityBindResponse,
        codex_app_server_protocol::IdentityErrorCode,
    > {
        // Synchronous phase (short lock): wire contract + time window.
        // The guard does NOT cross the await (StdMutexGuard is not Send).
        let (prepared, required) = {
            let guard = self
                .identity
                .lock()
                .map_err(|_| codex_app_server_protocol::IdentityErrorCode::AuthorityUnavailable)?;
            (guard.prepare(&proof)?, guard.required())
        };
        // Async phase (no lock): online verification against the authority.
        // Production without a wired verifier → ON = fail-closed.
        let verified = if required {
            let verifier = self
                .authority_verifier
                .as_deref()
                .ok_or(codex_app_server_protocol::IdentityErrorCode::IdentityUnverified)?;
            Some(
                verifier
                    .verify(prepared.proof(), prepared.challenge())
                    .await
                    .map_err(|refusal| {
                        tracing::warn!(?refusal, "identity authority verification refused bind");
                        // `challenge_mismatch` maps to AudienceMismatch;
                        // every other refusal stays fail-closed.
                        crate::identity::identity_error_for_refusal(&refusal)
                    })?,
            )
        } else {
            None
        };
        // Synchronous commit (replay re-check: a second concurrent bind
        // prepared before the commit fails here).
        let mut guard = self
            .identity
            .lock()
            .map_err(|_| codex_app_server_protocol::IdentityErrorCode::AuthorityUnavailable)?;
        guard.commit(prepared, verified)
    }
}
...
pub(crate) struct MessageProcessorArgs {
    ...
    /// Authority verifier. `None` = fail-closed when ON.
    pub(crate) authority_verifier: Option<std::sync::Arc<dyn AuthorityVerifier>>,
    /// Validated startup latch: enforcement is on only when the launcher required
    /// identity and the inherited channel was captured at startup.
    pub(crate) identity_required: bool,
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::IdentityBind { request_id, params } = &codex_request {
            ...
            let response = session
                .bind_identity(params.proof.clone(), self.authority_verifier.as_deref())
                .await
