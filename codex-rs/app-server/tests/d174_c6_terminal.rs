...
async fn auditor_first_mcp_spawn_on_bound_connection_scrubs_reserved_env() -> Result<()> {
    ...
    let script = r#"
const fs = require('fs');
fs.appendFileSync(process.env.AUDIT_CAPTURE, JSON.stringify({session:process.env.NEXUSCREW_MCP_SESSION ?? null,tmux:process.env.TMUX ?? null,pane:process.env.TMUX_PANE ?? null,verifiedVersion:process.env.NEXUSCREW_VERIFIED_ENV_VERSION ?? null,verifiedOwner:process.env.NEXUSCREW_VERIFIED_OWNER_INSTANCE_ID ?? null,verifiedCell:process.env.NEXUSCREW_VERIFIED_CELL_ID ?? null,verifiedIncarnation:process.env.NEXUSCREW_VERIFIED_INCARNATION_ID ?? null,verifiedBinding:process.env.NEXUSCREW_VERIFIED_BINDING_ID ?? null,verifiedOrigin:process.env.NEXUSCREW_VERIFIED_ORIGIN ?? null,verifiedThread:process.env.NEXUSCREW_VERIFIED_THREAD_ID ?? null})+'\n');
...
"#;
    ...
    let binding_id = client
        .binding_id()
        .context("bound client has a binding id")?;
    let thread_id = response["thread"]["id"]
        .as_str()
        .context("thread/start returns the thread id")?;
    let expected = json!({
        "session":null,
        "tmux":null,
        "pane":null,
        ...
        "verifiedCell":"cell-a",
        "verifiedIncarnation":"incarnation-a",
        "verifiedBinding":binding_id,
        "verifiedOrigin":"local_tui",
        "verifiedThread":thread_id,
    });
    assert!(
        values.iter().all(|v| *v == expected),
        "every shared verified spawn must scrub config identity and carry binding metadata, including the first one: {values:?}"
    );
    Ok(())
}

#[ignore = "sandboxed MCP spawn needs user namespaces; run in CI"]
#[tokio::test]
async fn unbound_mcp_spawn_never_carries_verified_metadata() -> Result<()> {
    let capture = tempfile::tempdir()?;
    let capture_path = capture.path().join("spawn.jsonl");
    let script = r#"
const fs = require('fs');
...
"#;
        ...
        .request(
            ...
            Some(json!({
                ...
                "config": {"mcp_servers": {"audit_probe": {
                    ...
                    "startup_timeout_sec": 2, "required": true,
                    "env":{"AUDIT_CAPTURE":capture_path.to_string_lossy(),
                        "NEXUSCREW_VERIFIED_CELL_ID":"configured-value"}
                }}}
            })),
        )
        .await?;
    ...
    assert!(
        response.get("error").is_none(),
        "unbound thread/start failed: {response}"
    );
    let records =
        std::fs::read_to_string(capture_path).context("real MCP process must record startup")?;
    println!("mcp_spawn_records={records}");
    ...
    for name in [
        ...
        assert!(
            values.iter().all(|v| v[name].is_null()),
            "unbound spawn must leave {name} absent even when config injects it: {values:?}"
        );
    }
    Ok(())
}
...
#[ignore = "sandboxed MCP spawn needs user namespaces; run in CI"]
#[tokio::test]
async fn unbound_mcp_spawn_rejects_reserved_verified_env_var_override() -> Result<()> {
    let fixture = IdentityFixture::start().await?;
    let mut client = fixture.connect_unbound().await?;
    let response = client
        .request(
            ...
            Some(json!({
                "ephemeral": true,
                "config": {"mcp_servers": {"audit_probe": {
                    "command":"node", "args":["-e", "process.stdout.write('')"],
                    "startup_timeout_sec": 2, "required": true,
                    "env_vars": ["NEXUSCREW_VERIFIED_CELL_ID"]
                }}}
            })),
        )
        .await?;
    ...
    let message = response["error"]["message"]
        .as_str()
        .context("thread/start must fail with an error message")?;
    assert!(
        message.contains("is reserved"),
        "reserved verified env override must fail closed: {response}"
    );
    Ok(())
}
...
async fn thread_binding_rejects_unbound_resume_and_fork() -> Result<()> {
    ...
    let resume_rejected = unbound
        ...
        .await?;
    assert_eq!(resume_rejected["error"]["message"], "IDENTITY_UNVERIFIED");
    ...
    let fork_rejected = unbound
        ...
        .await?;
    assert_eq!(fork_rejected["error"]["message"], "IDENTITY_UNVERIFIED");
...
async fn thread_binding_persistence_rejects_other_owner_and_unbound_after_restart() -> Result<()> {
    ...
    let rejected_unbound = unbound
        ...
        .await?;
    assert_eq!(rejected_unbound["error"]["message"], "IDENTITY_UNVERIFIED");
...
async fn auditor_bound_thread_path_resume_requires_owner() -> Result<()> {
    ...
    assert_eq!(via_path_unbound["error"]["message"], "IDENTITY_UNVERIFIED");
    Ok(())
}
