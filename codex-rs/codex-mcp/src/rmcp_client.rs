...
use codex_rmcp_client::SharedVerifiedIdentity;
...
async fn start_server_task(
    ...
    // Fork retry (codex-vl): some MCP servers are flaky on the first tools/list
    // at startup; retry up to MCP_STARTUP_TOOLS_LIST_ATTEMPTS before giving up.
    // Adapted to upstream's is_codex_apps_mcp_server signature; fetch_ticket is
    // bound before the loop so the downstream cache publish still consumes it.
    // Cache source renamed upstream 0.145 (CodexAppsToolsFetchSource →
    // ConnectorRuntimeFetchSource); retry loop preserved per fork carry-forward.
        ...
        McpServerTransportConfig::Stdio {
            ...
            RmcpClient::new_stdio_client_with_protocol_mode_and_binding(
                ...
                runtime_context
                    .binding_context()
                    .cloned()
                    .map(SharedVerifiedIdentity::from),
            )
            .await
