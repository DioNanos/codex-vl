...
#[tokio::test]
async fn get_model_info_with_custom_catalog_keeps_bundled_models_available() {
    let codex_home = tempdir().expect("temp dir");
    let config = ModelsManagerConfig::default();
    let auth_manager = AuthManager::from_auth_for_testing(CodexAuth::from_api_key("Test API Key"));
    let custom_catalog = ModelsResponse {
        models: vec![remote_model("glm-5.1:cloud", "GLM 5.1 Cloud", 50)],
    };
    let manager = ModelsManager::new(
        codex_home.path().to_path_buf(),
        auth_manager,
    ...
    let gpt = manager.get_model_info("gpt-5.4", &config).await;
    let custom = manager.get_model_info("glm-5.1:cloud", &config).await;

    assert_eq!(gpt.slug, "gpt-5.4");
    assert!(!gpt.used_fallback_model_metadata);
    assert_eq!(custom.slug, "glm-5.1:cloud");
    assert!(!custom.used_fallback_model_metadata);
}
...
#[tokio::test]
async fn custom_catalog_keeps_remote_refresh_enabled() {
    let server = MockServer::start().await;
    ...
    assert!(available.iter().any(|model| model.model == "glm-5.1:cloud"));
    assert!(
        available
            .iter()
            .any(|model| model.model == "gpt-5.3-codex-spark"),
        "remote model should be merged even when a custom catalog is configured"
    );
    assert_eq!(
        models_mock.requests().len(),
        1,
        "expected one /models request"
    );
}

#[tokio::test]
async fn remote_refresh_overrides_bundled_slug_but_preserves_custom_only_slug() {
    let server = MockServer::start().await;
    let mut remote_builtin = remote_model("gpt-5.4", "Remote GPT 5.4", /*priority*/ -500);
    remote_builtin.description = Some("Remote override".to_string());
    let remote_custom_collision =
        remote_model("glm-5.1:cloud", "Remote Collision", /*priority*/ -400);
    let models_mock = mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_builtin.clone(), remote_custom_collision],
        },
    )
    .await;
    ...
    let gpt = available
        ...
        .expect("bundled slug should remain present");
    let custom = available
        .iter()
        .find(|model| model.model == "glm-5.1:cloud")
        .expect("custom slug should remain present");

    assert_eq!(gpt.display_name, "Remote GPT 5.4");
    assert_eq!(custom.display_name, "Local GLM");
    assert_eq!(
        models_mock.requests().len(),
        1,
        "expected one /models request"
