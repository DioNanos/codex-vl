...
pub struct ModelsManager {
    ...
    bundled_slugs: BTreeSet<String>,
    local_catalog_slugs: BTreeSet<String>,
...
impl ModelsManager {
    ...
    ///
    /// Uses `codex_home` to store cached model metadata and initializes with the bundled catalog
    /// overlaid with any caller-provided local catalog.
    pub fn new(
        codex_home: PathBuf,
    ...
    pub fn new_with_provider(
        ...
        let bundled_slugs = bundled_models
            .iter()
            .map(|model| model.slug.clone())
            .collect::<BTreeSet<_>>();
        let local_catalog_models = model_catalog
            .map(|catalog| catalog.models)
            .unwrap_or_default();
        let local_catalog_slugs = local_catalog_models
            .iter()
            .map(|model| model.slug.clone())
            .collect::<BTreeSet<_>>();
        let base_models = Self::merge_model_lists(bundled_models, local_catalog_models);
        Self {
            remote_models: RwLock::new(base_models.clone()),
            base_models,
            bundled_slugs,
            local_catalog_slugs,
            collaboration_modes_config,
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        let mut existing_models: BTreeMap<String, ModelInfo> = self
            .base_models
            .iter()
            .cloned()
        ...
        for model in models {
            ...
            if self.should_remote_override(&slug) {
                existing_models.insert(slug, model);
            } else {
                existing_models.entry(slug).or_insert(model);
            }
        }
        *self.remote_models.write().await = existing_models.into_values().collect();
    }
    ...
    fn merge_model_lists(base: Vec<ModelInfo>, overlay: Vec<ModelInfo>) -> Vec<ModelInfo> {
        let mut merged: BTreeMap<String, ModelInfo> = base
            .into_iter()
            .map(|model| (model.slug.clone(), model))
            .collect();
    ...
    fn should_remote_override(&self, slug: &str) -> bool {
        self.bundled_slugs.contains(slug) || !self.local_catalog_slugs.contains(slug)
    }
