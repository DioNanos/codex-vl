...
impl RemoteAppServerClient {
    ...
    /// Remote attach with an authority-verified identity proof. The
    /// proof is bound right after the initialize handshake and refused
    /// connections never attach unbound.
    pub async fn connect_with_verified_identity(
        ...
        match args.endpoint {
            ...
            RemoteAppServerEndpoint::UnixSocket { socket_path } => {
                // il daemon fleet e' dello stesso utente: su windows si
                // impone il peer non elevato, su unix il check non esiste.
                #[cfg(windows)]
