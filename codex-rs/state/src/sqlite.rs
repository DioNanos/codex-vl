...
// Fork: runtime.rs imports these by name for the VL migration paths, so they stay
// pub(crate). QUEUE_DB_FILENAME is new upstream and reached only through
// `queue_db_path()`, so it keeps upstream's private visibility.
pub(crate) const LOGS_DB_FILENAME: &str = "logs_2.sqlite";
pub(crate) const GOALS_DB_FILENAME: &str = "goals_1.sqlite";
pub(crate) const MEMORIES_DB_FILENAME: &str = "memories_1.sqlite";
const QUEUE_DB_FILENAME: &str = "queue_1.sqlite";
pub(crate) const STATE_DB_FILENAME: &str = "state_5.sqlite";
pub(crate) const THREAD_HISTORY_DB_FILENAME: &str = "thread_history_1.sqlite";
...
impl SqliteConfig {
    ...
    // codex-vl owns state opening in `runtime` so it can reconcile legacy loop
    // migrations and retry Android-compatible SQLite settings. Keep the upstream
    // helper for parity and test coverage.
    #[allow(dead_code)]
