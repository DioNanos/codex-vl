...
fn update_action_label(context: &InstallContext) -> &'static str {
    // codex-vl: fork-owned npm package name preserved through the
    // upstream `InstallContext { method, package_layout }` migration.
    match &context.method {
        InstallMethod::Npm => "npm install -g @mmmbuto/codex-vl",
        InstallMethod::Bun => "bun install -g @mmmbuto/codex-vl",
        InstallMethod::VitePlus => "vp install -g @mmmbuto/codex-vl",
...
mod tests {
    ...
    fn update_action_labels_install_contexts() {
        // codex-vl fork: package name preserved through InstallContext
        // struct migration; Npm label still points to @mmmbuto/codex-vl.
        assert_eq!(
