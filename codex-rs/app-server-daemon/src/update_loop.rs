...
async fn adopt_managed_updater(
    ...
    {
        let _ = listener;
        reexec_managed_updater(&managed_bin, &daemon.install_context)
...
#[cfg(unix)]
/// The fork must never replace itself with an upstream package shim. These
/// installs are owned by the fork package, so updater replacement is a no-op.
...
async fn install_latest_standalone() -> Result<()> {
    anyhow::bail!(
        "codex-vl fork: standalone auto-updater is disabled. The fork does \
         not fetch the upstream openai/codex install script. To update the \
         fork, run: npm install -g @mmmbuto/codex-vl@latest"
    )
}
