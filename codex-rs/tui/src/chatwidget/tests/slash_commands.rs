...
async fn slash_vl_alias_falls_back_to_direct_vivling_chat_without_brain() {
    ...
    assert!(
        rendered.contains("I'm ") || rendered.contains("As "),
        "expected direct Vivling reply via /vl alias, got {rendered:?}"
    );
    assert!(
        !rendered.contains("Unrecognized command"),
        "expected /vl to parse as Vivling alias, got {rendered:?}"
    );
}
...
async fn slash_vl_alias_dispatches_brain_chat_when_adult_profile_is_ready() {
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        rendered.contains("Vivling brain is thinking"),
        "expected thinking status after dispatch, got {rendered:?}"
    );
}
