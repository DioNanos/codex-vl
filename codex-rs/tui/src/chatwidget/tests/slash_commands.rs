...
async fn slash_mcp_reload_emits_global_reload_vl_event() {
    ...
    // codex-vl: upstream 0.156 now emits extra app events during submit
    // (FollowTranscript from config persistence); drain non-VL events until
    // the fork McpReload event arrives. The invariant stands: McpReload on
    // the app event channel with the right thread id.
    let mut mcp_reload_seen = false;
    loop {
        match rx.try_recv() {
            Ok(AppEvent::Vl(VlEvent::McpReload {
                thread_id: actual_thread_id,
            })) if actual_thread_id == thread_id => {
                mcp_reload_seen = true;
                break;
            }
            Ok(_) => continue,
            Err(_) => break,
    ...
    assert!(
        mcp_reload_seen,
        "expected AppEvent::Vl(VlEvent::McpReload) with thread_id {thread_id}"
    );
