...
impl ChatWidget {
    ...
    fn pending_start_warn_threshold(&self) -> std::time::Duration {
        static SECS: std::sync::OnceLock<u64> = std::sync::OnceLock::new();
        let secs = *SECS.get_or_init(|| {
            std::env::var("CODEX_TUI_PENDING_START_WARN_SECS")
                .ok()
    ...
    pub(crate) fn pre_draw_tick(&mut self) {
        ...
        self.check_pending_start_watchdog_with(self.pending_start_warn_threshold());
    ...
    pub(crate) fn prepare_local_op_submission(&mut self, op: &AppCommand) {
        if matches!(
            ...
            self.input_queue.user_turn_pending_since = Some(std::time::Instant::now());
