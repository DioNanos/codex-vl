...
async fn cached_legacy_resume_revalidates_history_across_migration_settings() -> Result<()> {
        ...
        [(false, false), (false, true), (true, false), (true, true)]
    {
        ...
        // The startup worker may win the maintenance-lock race and migrate the fixture before
        // resume revalidates; then resume takes the paginated path with a different request
        // count. The cached-legacy count only holds when the thread is still Legacy after
        // resume, so gate the exact assertion on that.
        let still_legacy = app_server
            .history_pagination
