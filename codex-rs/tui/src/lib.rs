...
pub(crate) async fn connect_remote_app_server_with_identity(
    endpoint: RemoteAppServerEndpoint,
    proof: Option<codex_app_server_protocol::IdentityProof>,
) -> color_eyre::Result<AppServerClient> {
    let app_server = RemoteAppServerClient::connect_with_identity(
...
async fn connect_remote_app_server(
    ...
    connect_remote_app_server_with_identity(endpoint, /*proof*/ None).await
}
    ...
    Fut: Future<Output = std::io::Result<InProcessAppServerClient>>,
{
    ...
    let client = start_client(InProcessClientStartArgs {
        ...
        mcp_server_openai_form_elicitation: false,
        opt_out_notification_methods: Vec::new(),
        identity_channel: codex_app_server_client::prepared_identity_channel(),
        channel_capacity: DEFAULT_IN_PROCESS_CHANNEL_CAPACITY,
    })
    .await
...
pub mod identity_gate_test_support {
    //! Thin test facade over the production launch-target helpers. The gate
    //! itself lives in `app_server_target_for_launch`; this module only exposes
    //! the surface the attach identity gate tests need.

    use super::AppServerTarget;
    use super::IDENTITY_REQUIRED_ENDPOINT_DIAGNOSTIC;
...
async fn run_ratatui_app(
    ...
    fleet_identity_proof: Option<codex_app_server_protocol::IdentityProof>,
