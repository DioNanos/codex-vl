...
pub(super) async fn reconnect(
    ...
    for delay in [0, 1, 2, 4, 8] {
        let attempt = async {
            ...
            // Reconnect carries a fresh proof: never re-attach to a bound
            // endpoint without a verified identity.
            let identity_proof = crate::load_nexuscrew_identity_proof().await;
            let client = crate::app_server_connection::connect(&target, identity_proof).await?;
