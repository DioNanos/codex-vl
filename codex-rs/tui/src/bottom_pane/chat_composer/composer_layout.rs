...
impl ChatComposer {
    pub(super) fn layout_with_options(
        ...
        // codex-vl: same fork hook as desired_height_with_options — the VL
        // footer lane reserves its row in the LAYOUT too, or split_vl_footer_area
        // would never find room to render it.
        let footer_hint_height =
            self.footer_height_with_vl_context(self.footer_hint_height(area.width, options));
    ...
    pub(crate) fn desired_height_with_options(
        ...
        // codex-vl: the VL footer lane adds one row below the upstream footer
        // when loop/Vivling context is present (no-op otherwise). Upstream's
        // composer_layout refactor moved the desired-height accounting here;
        // keep the fork hook aligned with layout_areas_with_textarea_right_reserve.
        let footer_hint_height =
            self.footer_height_with_vl_context(self.footer_hint_height(width, options));
