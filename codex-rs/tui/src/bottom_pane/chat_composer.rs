...
// codex-vl: extra spacing row between the configured footer and the VL context lane.
...
impl ChatComposer {
    ...
    fn layout_areas_with_textarea_right_reserve(
        ...
        let base_footer_hint_height = self
            .custom_footer_height()
            .unwrap_or_else(|| footer_height(&footer_props, area.width));
        let footer_hint_height = self.footer_height_with_vl_context(base_footer_hint_height);
    ...
    pub(crate) fn render_with_options(
        ...
        match options.footer {
            ...
            None => {
                ...
                let base_footer_hint_height =
                    custom_height.unwrap_or_else(|| footer_height(&footer_props, popup_rect.width));
                let footer_hint_height =
                    self.footer_height_with_vl_context(base_footer_hint_height);
                ...
                let (hint_rect, vl_context_rect) = self.split_vl_footer_area(hint_rect);
                ...
                self.render_vl_footer_context(vl_context_rect, buf);
            }
        }
