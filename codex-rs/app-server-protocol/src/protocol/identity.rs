...
/// Numeric version used by the verify-channel v1 wire contract.
pub const IDENTITY_VERIFY_VERSION: u64 = 1;
...
fn valid_epoch_ms(value: i64) -> bool {
    value >= 0
}
...
impl IdentityClaims {
    pub fn validate(&self) -> Result<(), IdentityErrorCode> {
        ...
        if !valid_epoch_ms(self.issued_at)
            || !valid_epoch_ms(self.not_before)
            || !valid_epoch_ms(self.expires_at)
            || self.not_before > self.expires_at
            || self.expires_at <= self.issued_at
        {
            return Err(IdentityErrorCode::InvalidTime);
    ...
    fn fixture() -> IdentityProof {
        IdentityProof {
            ...
            claims: IdentityClaims {
                ...
                issued_at: 1_757_030_400_000,
                not_before: 1_757_030_400_000,
                expires_at: 1_757_030_460_000,
                nonce: "a".repeat(64),
                thread_id: None,
                cwd: None,
                live_host: None,
