...
The npm `latest` tag tracks the `0.136.1` line, which merges upstream Codex
`rust-v0.136.0` while preserving the Codex VL workflow layer. The `stable`
dist-tag points to the previous `0.135.0` line for conservative installs. Both
ship Linux x64, Linux arm64 (musl) and Android arm64 native packages plus a
macOS arm64 source-build package, each platform under its own `<platform>`
dist-tag (`linux-x64`, `linux-arm64`, `android-arm64`, `darwin-arm64`).

```bash
npm install -g @mmmbuto/codex-vl@latest   # 0.136.1
